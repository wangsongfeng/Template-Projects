...
+(NSString *)base64StringFromText:(NSString*)text
{
    if(text && ![text isEqualToString:LocalStr_None]){
        ...
        //IOS 自带DES加密 End
        return [self base64EncodedStringFrom1:data];
...
{
        if((sText == nil || sText.length == 0)||(key==nil||key.length==0)||(ivDes==nil||ivDes.length==0)){
            return@"";
        }
        //gb2312 编码
        ...
        CCCryptorStatus ccStatus = (CCCryptorStatus)nil;
...
{
        if (string == nil)
                [NSException raise:NSInvalidArgumentException format:@"%@", string];
        if ([string length] == 0)
                return [NSData data];
...
/******************************************************************************
  ...
  备注信息 :
  ******************************************************************************/
+ (NSString *)base64EncodedStringFrom1:(NSData *)data
{
        if ([data length] == 0)
                return @"";
...
//	IOS	Hmac_sha1 加密http://blog.woodbunny.com/post-86.html
//http://wenku.baidu.com/link?url=p9dojKXYjdHdg5qFsvjSeqWXAP_AlVEsf8OZSLHWj_Qar4J2upAmaeAFmh9a781RPpwgUxq0bxg7JdkZRITjyE9FWSxSagT8JQaQBgslCpS
//IOS	Hmac_sha512加密实现原理
...

{
    ...
    uint8_t cHMAC[CC_SHA512_DIGEST_LENGTH];//CC_SHA1_DIGEST_LENGTH
