...
int  DB_DES(char *inbuf, int inlen, char *outbuf, int *outlen, int flag)
{
    ...
    if(flag == 0)   /* Encrypt */
    {
        ...
        Hex2Str(in,  tmp, 8);
        Des((unsigned char *)KEY, (unsigned char *)tmp, (unsigned char *)out);
    }
    else
    {
        /* DisEncrypt */
        ...
        _Des((unsigned char *)KEY, (unsigned char *)in, (unsigned char *)tmp);
        Str2Hex(tmp,  out, 16);
    }
...
#else
    TDes((unsigned char *)key2, (unsigned char *)desin, (unsigned char *)desout);
#endif
...
void decrpt3DES(char *outbuf,char *data,char *myklk)
{
    ...
    strcpy(desin, data);
    strcpy(key1,myklk);
    _TDes((unsigned char *)key1, (unsigned char *)desin, (unsigned char *)outbuf);
}
...
void encrpt3DES(char *outbuf,char *data,char *myklk)
{
    ...
    strcpy(desin, data);
    strcpy(key1,myklk);
    TDes((unsigned char *)key1, (unsigned char *)desin, (unsigned char *)outbuf);
}
...
//    int dataLen = strlen (data);
    ...
    Des((unsigned char *)myklk, (unsigned char *)data, (unsigned char *)outbuf);
//    printf ("outbuf = %s\n",outbuf);
}
...
//    int dataLen = (int)strlen(data);
    memset (outbuf,0x00,30);
    _Des((unsigned char *)myklk, (unsigned char *)data, (unsigned char *)outbuf);
}
